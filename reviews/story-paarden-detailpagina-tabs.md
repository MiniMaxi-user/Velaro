# Story — Herstructureer paarden-detailpagina (staleigenaar) naar tab-layout met 70/30 kolommen

Type: VERBETERING (UI-herstructurering, geen datamodel-wijziging)
Status: **Nog niet bouwen** — alleen verhelderd en op het bord gezet.

---

## Doel

De paarden-detailpagina (`/paarden/[id]`) toont nu één lange kolom met losse
...
- Betreft de weergave voor **stalleden** (OWNER en STAFF) — d.w.z. `canEdit === true`.
...
- **Rechterkolom (30%) — altijd zichtbaar**, ongeacht de actieve tab:
  1. **Identificatie** (UELN, chipnummer, paspoortnummer) — verhuist hierheen vanuit
     de huidige hoofdkolom.
  2. Daaronder: **Welzijn / EU** (slachtuitsluiting + datum).

- **Linkerkolom (70%) — tabstrip** met de volgende tabbladen:

  | Tab | Inhoud |
  |-----|--------|
  | **Algemeen** | De huidige panelen **Algemeen** én **Afstamming** samengevoegd in deze tab. |
  | **Gezondheid** | De bestaande `GezondheidTabs` met de sub-tabs Vaccinaties, Ontworming, Dierenarts, Hoefsmit (ongewijzigd, één niveau dieper genest onder deze tab). |
...
## Verduidelijkingen / open punten (vóór bouwen beantwoorden)
...
2. **"Alle gegevens van de eigenaren":** het `User`-model bevat momenteel alleen
   ...
   adres) — dat zou een datamodel-uitbreiding zijn.
...
5. **Responsive gedrag:** op smalle schermen klapt het 70/30-grid naar één kolom
...
- `src/app/(app)/paarden/[id]/page.tsx` — layout omzetten naar 70/30 grid + tabs;
  Identificatie en Welzijn/EU naar de rechterkolom.
- Nieuw: `src/features/paarden/PaardDetailTabs.tsx` (client) — tabstrip + actieve-tab-state.
- Hergebruik: `GezondheidTabs`, `VoederschemaPanel`, `BerichtenPanel`, `EigenaarBeheer`.
- `src/features/paarden/EigenaarBeheer.tsx` — pen-icoon per eigenaar toevoegen (link
  naar bewerkpagina) naast de bestaande ontkoppel-knop.
- `src/styles/globals.css` — tab-styling + 70/30-grid (hergebruik bestaande tokens;
...
5. Tab Eigenaren toont per eigenaar de gegevens, een pen-icoon naar de
   eigenaar-bewerkpagina, én de bestaande mogelijkheid om eigenaren te
