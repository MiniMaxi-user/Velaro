...
secties: "Openstaand — N" en "Gedaan — N". Ik weet dus hoeveel taken er open
...
"Openstaand — 3" en "Gedaan — 9" optellen en delen om te beseffen dat ik op
75% zit.
...
de datumnavigatie geeft in een oogopslag: "9 van 12 taken gedaan — 75%".
...
- Consistentie met het dashboard: het staldashboard toont al een percentage
  op de KPI-kaart "Taken afgerond". De takenpagina zelf — het meest gebruikte
  scherm voor dagelijkse taakopvolging — zou hetzelfde signaal moeten geven.
...
Wanneer voortgangPct === 100 toont het label "N/N gedaan — alles afgerond"
...
- src/app/(app)/stal/taken/page.tsx — voortgangsberekening toevoegen (3 regels
  per modus) en de balk-JSX renderen op twee plaatsen (specifieke stal en alle
  stallen, circa 12 regels per modus)
- src/styles/globals.css — vijf nieuwe CSS-klassen toevoegen (circa 25 regels)
...
2. Naast de balk staat een tekst "X/Y gedaan — Z%" die het percentage weergeeft.
...
4. Wanneer alle taken afgevinkt zijn (100%), kleurt de balk groen en verandert
   het label naar "N/N gedaan — alles afgerond".
