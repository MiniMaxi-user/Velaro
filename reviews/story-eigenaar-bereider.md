...
één of meer **bereiders** — de persoon/personen die het paard rijden. Een bereider is
vaak een **minderjarige** en heeft dus géén eigen Velaro-account. Vandaag kent het
model alleen `HorseOwner` (account-gebonden eigenaar). We breiden het centrale
paardprofiel uit zodat de stal eigenaar én bereider(s) kan vastleggen en beheren, en
...
## Datamodel (Prisma — schemawijziging toegestaan)
Nieuw model, géén accountkoppeling (een bereider kan minderjarig zijn en heeft geen login):
...
> apart opgeslagen. Geen geboortedatum → geen badge.
...
- **Bereider volledig beheren** (toevoegen, alle velden wijzigen, verwijderen):
  alleen staf/eigenaar van de stal → `getStableRole(user.id, horse.stableId) !== null`.
- **Telefoonnummer bereider wijzigen**: óók de paardeigenaar (`isHorseOwner`), maar
...
- `addHorseRider(horseId, formData)` — staf only; valideert `name` verplicht.
- `updateHorseRider(horseId, riderId, formData)` — staf only; werkt alle velden bij.
- `updateRiderPhone(horseId, riderId, formData)` — staf **of** paardeigenaar; schrijft
  **alleen** `phone`. Controleer eigenaarschap met `isHorseOwner`.
- `removeHorseRider(horseId, riderId)` — staf only.

## Queries (`src/features/paarden/queries.ts`)
- `getHorse`: include `riders: { orderBy: { createdAt: 'asc' } }` (naast bestaande `owners`).

## UI — staf/eigenaar (detailscherm, tab)
...
## UI — paardeigenaar (zijn eigen paardpagina)
...
- Nieuw component `src/features/paarden/BereiderInfo.tsx` (client) dat eigenaar(s) en
  ...
  één **inline bewerkbaar telefoonnummer** (klein formuliertje → `updateRiderPhone`).
...
6. Server-side afgedwongen: een niet-staf gebruiker kan via `updateRiderPhone` alléén
