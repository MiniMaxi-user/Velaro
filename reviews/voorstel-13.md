...
- Vaccinatie Griep — volgende datum: 14 jan 2026 (al 5 maanden verlopen) → oranje
- Ontworming — volgende datum: 15 aug 2026 (over 2 maanden) → zelfde oranje
- Hoefsmit — volgende datum: 3 sep 2026 (ver weg) → zelfde oranje
...
Het dashboard-panel "Aankomende zorg" doet dit onderscheid wél goed (badge
...
- Gebruiksgemak is een van de vijf succesfactoren in het businessplan. Een
  ...
  signaal — dat is het tegenovergestelde van gebruiksgemak.
- Pensionstallen (beachhead-doelgroep) beheren tientallen paarden tegelijk.
  Een staleigenaar die in één oogopslag ziet welke datums verlopen zijn, kan
...
- Mobiele ervaring (succesfactor): kleurcodering werkt op elk schermformaat
  en vermindert de cognitieve belasting — de gebruiker hoeft geen mentale
...
Scope: één component (GezondheidTabs.tsx) en één kleine CSS-uitbreiding.
...
- verlopen: nextDate ligt vóór vandaag
...
Vervang of breid `.gezondheid-next` uit (huidige regel ~1730):
    ...
    /* Volgende datum in gezondheidsoverzicht — statuskleur */
    .gezondheid-datum {
      font-size: 12px;
      font-weight: 500;
    }
...
- src/features/gezondheid/GezondheidTabs.tsx — getDatumStatus helper +
  DatumBadge component + drie plaatsen waar de datum wordt getoond
- src/styles/globals.css — drie .gezondheid-datum--* klassen toevoegen,
...
- Eén component uitbreiden met een twee-regels helper.
...
4. Wanneer geen nextDate is ingevuld, blijft het streepje (—) zichtbaar in
   de muted kleur.
