...
lege staat "Je bent nog niet aan een stal gekoppeld." — ongeacht het
...
- Bouwvolgorde stap 5 in CLAUDE.md: "Eigenaarscommunicatie + gedeeld
  profiel" — het gedeelde profiel bestaat al (paardenprofiel is leesbaar
...
Scope: één nieuwe pagina (/eigenaar), een redirect in de app-root,
...
(bestaat al in src/features/mededelingen/queries.ts) — beperkt tot 2
...
stable === null en toont dan een lege staat. Voeg vóór die check een
controle toe: als de gebruiker géén StableMember is maar wél
...
5. Een gebruiker die zowel StableMember als HorseOwner is (bijv. een
   staleigenaar met eigen paard) blijft op /stal — de redirect geldt
