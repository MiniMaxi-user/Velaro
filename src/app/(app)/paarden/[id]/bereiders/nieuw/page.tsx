...
import { addHorseRider } from '@/features/paarden/actions'
...
export default async function BereiderNieuwPage({ params }: Props) {
  ...
  const action = addHorseRider.bind(null, id)
  ...
  return (
    <main className="page-container">
      ...
      <div style={{ marginBottom: 'var(--velaro-space-8)' }}>
        <div className="label">Bereider toevoegen</div>
        <h1 className="page-title">{horse.name}</h1>
      </div>

      <BereiderForm horseId={id} action={action} />
    </main>
  )
}
